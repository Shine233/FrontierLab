...
- 复现状态：VGGT-Ω / Depth Anything 3 / Pi3 / LingBot-Map inference 复现 planned；MapAnything / HunyuanWorld-Mirror planned。
...
- 关联：VGGT-Ω 更适合作为强 RGB-only/video geometry backbone 和空间 token 表征候选；Depth Anything 3 更适合作为任意视角视觉几何/NVS backbone；Pi3 更适合作为无序多视图/reference-free robustness baseline；LingBot-Map 更适合作为连续视频在线建图/VO 候选；MapAnything/HunyuanWorld-Mirror 更适合多先验/metric reconstruction 或重渲染分支。
- 复现：VGGT-Ω / Depth Anything 3 / Pi3 / LingBot-Map inference-level sanity check planned。
