...
| 图像匹配方法横向对比 | image-matching / visual-localization / 3d-reconstruction / evaluation-benchmarks | RoMa / RoMa v2 / LightGlue / GIM / LoMa | initial-completed | [`comparisons/image-matching/image-matching-methods.md`](../comparisons/image-matching/image-matching-methods.md) | 稠密路线选 RoMa v2（精度上限、难/跨域 EPE 大降）；稀疏路线选 LightGlue（速度/工程）；要跨域泛化套 GIM；LoMa 是稀疏 scaling 新作但训练不可复现。MegaDepth-1500/ScanNet-1500/ZEB 可比数字见报告第 3 节。 |
...
| XPeng X 系列自动驾驶世界模型横向对比 | world-models / robotics-autonomous-driving / generation-diffusion / efficient-training-inference | X-World / X-Cache / X-Foresight / Xiaomi Auto World Model | initial-completed | [`comparisons/world-models/xpeng-x-series-world-models.md`](../comparisons/world-models/xpeng-x-series-world-models.md) | X-World 是可控 7 相机闭环仿真器；X-Cache 是 few-step AR DiT 推理加速层；X-Foresight 把预测式世界模型接入 VLA/LDM；三者目前均无公开代码/权重/数据。 |
| Robot WAM / video-diffusion robot policies 横向对比 | world-models / robotics-autonomous-driving / generation-diffusion / evaluation-benchmarks | DreamZero / GR00T N1.x / pi_0 / pi_0.5 / V-JEPA 2 / Genie / X-Foresight | initial-completed | [`comparisons/world-models/robot-world-action-models.md`](../comparisons/world-models/robot-world-action-models.md) | DreamZero 是当前最可优先复现的 robot WAM 路线；它公开代码、权重、DROID 数据与训练脚本，但 AgiBot 主数据和真实机器人环境不可完整复刻。 |
