...
- 符号： $p_s$ 是 student 对某个 crop 的 `[CLS]` token 经 head + softmax 得到的分布， $p_t$ 是 teacher 对 global crop 的对应分布， $k$ 遍历 prototype 维度（128k）。
...
- 符号： $i$ 遍历被 mask 的 patch 位置集合 $\mathcal{M}$ ； $p_{s,i}$ 是 student 在被遮 patch $i$ 上的预测分布， $p_{t,i}$ 是 teacher 在**未遮**的同位置 patch 上的分布。
...
- 符号： $n$ 是 batch 内样本数， $x_i$ 是 L2 归一化后的特征， $d_{n,i}$ 是样本 $i$ 到 batch 内最近邻的距离。
