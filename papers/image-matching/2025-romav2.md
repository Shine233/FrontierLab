...
- 符号： $z^A\_m$ 、 $z^B\_n$ 分别是图 A 第 $m$ 个、图 B 第 $n$ 个 patch 的匹配特征； $\mathrm{cossim}$ 为余弦相似度； $\tau = 1/10$ 为温度。
...
- 符号： $\mathcal{L}\_{\text{warp}}$ 为回归 warp 的 Charbonnier 损失（见公式 4）； $\mathcal{L}\_{\text{overlap}}$ 监督可见/重叠区域的置信度； $10^{-2}$ 是 overlap 项权重； $p\_{\text{GT}}$ 为真值。
