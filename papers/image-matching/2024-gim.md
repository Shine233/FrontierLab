...
- 符号： $f_\theta$ 是被增强的 backbone； $\mathcal{L}_{\text{arch}}$ 是该架构**原本的**匹配损失（LoFTR 用 focal/coarse-fine、DKM/RoMa 用回归损失等），不新设计损失； $\mathcal{C}^{AB}$ 是传播 + 过滤后的对应标签，损失**只在有对应的像素上**回传； $\mathcal{D}$ 是原域数据与视频数据的**等概率混合**分布。
...
- 符号：对 12 个跨域数据集分别算相对位姿误差的 AUC@5°（由本质矩阵 + RANSAC 估位姿），再取均值；位姿误差 $e$ 取旋转角误差 $e_{\text{rot}}$ 与平移角误差 $e_{\text{trans}}$ 的**较大者**。
