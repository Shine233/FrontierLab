...
- **公式 (2)：局部特征相似度**
  ...
  - 符号： $F^{1,1}_i, F^{2,1}_j$ 是两图第 $i, j$ 像素的 $\ell_2$ 归一化局部特征； $\tau$ 是温度（ $\tau = 0.07$ ）；内积越大表示越相似。
...
- **公式 (5)：快速互易匹配（FRM）的迭代**
  ...
  - 符号： $U^t \subset \mathcal{P}^1$ 是第 $t$ 轮图 1 的候选像素集（初始 $U^0$ 为规则网格采样的 $k$ 个点）； $NN_2(\cdot)$ 是在图 2 特征里取最近邻、 $NN_1(\cdot)$ 反向； $U^t = U^{t+1}$ 的不动点即为互易匹配。
