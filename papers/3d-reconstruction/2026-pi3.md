...
- **输入 / 输出**：输入为 $N$ 张 RGB 图像；输出为每张图对应的 camera pose $T\_i$ 、pixel-aligned local point map $X\_i$ 和 confidence map $C\_i$ 。
...
| Pose 解码头 | 5 层轻量 transformer（仅 per-image self-attention）→ MLP → average pooling → MLP，adapted from Reloc3r | 预测 9D 旋转（经 SVD 正交化为 3×3）+ 平移，输出相对位姿 |
...
三个解码头**不共享权重**，但**共享同一套架构**：一个只对单图内 patch token 做 self-attention 的 5 层轻量 transformer。每个头内部不再跨图交换信息，这保证了输出与输入 view 的一一对应关系不被破坏——这是置换等变性在解码端的具体落地。

几个已确认的实现细节值得强调：

- **旋转参数化**：pose 头先回归一个 9D 表示（Levinson et al. 2020 的连续旋转表示），再用 SVD 正交化投影为合法的 3×3 旋转矩阵。这比直接回归四元数/欧拉角更平滑、无奇异点，也不需要额外归一化约束。
- **上采样方式**：point map 头和 confidence 头都是「MLP + pixel shuffle」，而不是 DUSt3R/VGGT 常用的 DPT-style 多尺度解码。pixel shuffle 直接把通道维重排回空间分辨率，结构更轻，但也是原文承认高不确定区域会出现 grid-like artifacts 的直接原因（见 §6）。
- **pose 头的池化**：average pooling 出现在 pose 头内部（MLP → average pooling → MLP），用于把 per-patch 特征汇聚成单图的一个位姿向量，这与逐像素输出的 point/confidence 头形成对比。
...
置换等变最直观的收益是「换参考帧不换结果」。原文 Figure 2 用同一场景、不同 reference frame 选择做对比：VGGT / Fast3R / FLARE 这类带参考帧偏置的方法，指标随参考帧选择明显波动；π³ 因为架构里根本没有「参考帧」这个概念，无论选哪张图作起点，输出都稳定一致。这张图和 §4.1 里「打乱输入顺序时 Pi3 指标方差近 0」的定量证据（Table 6）是同一机制的两面。

![不同参考帧下的性能一致性（来源：arXiv 2507.13347v3, Pi3, Figure 2）：π³ 在任意参考帧下都稳定，参考帧方法则结果抖动](../../assets/3d-reconstruction/2026-pi3/consistency.png)
...
- 符号： $\hat{x}\_{i,j}$ 为第 $i$ 张图第 $j$ 个像素的预测 3D 点， $x\_{i,j}$ 为对应 GT， $z\_{i,j}$ 为 GT 深度（用作 depth-weighting 权重的分母，让远处点权重更小）， $N$ 为视图数， $HW$ 为每图像素数。
...
- 符号： $\hat{R}\_{i \leftarrow j}, \hat{t}\_{i \leftarrow j}$ 为预测相对旋转/平移， $R, t$ 为 GT， $H\_\delta$ 为 Huber 损失（对 outlier 更鲁棒）， $\lambda\_{\text{trans}}$ 平衡旋转与平移（论文取 100.0）， $s^\ast$ 复用点图那套全局尺度校正平移量纲。
...
**法向损失（normal loss）**：还有一项监督表面法向，最小化预测法向 $\hat{n}\_{i,j}$ 与 GT 法向 $n\_{i,j}$ 的夹角：

$$ L_{\text{normal}} = \frac{1}{NHW} \sum_{i=1}^{N} \sum_{j=1}^{HW} \arccos\!\big( \hat{n}_{i,j} \cdot n_{i,j} \big) $$

- 符号： $\hat{n}_{i,j}, n_{i,j}$ 均为单位法向量，点积即夹角余弦， $\arccos$ 还原为角度。
- 作用：直接约束局部几何的一阶结构（表面朝向），补充点图 L1 只约束位置的不足，鼓励更平整、少噪的重建表面。

**总损失**：

$$ L = L_{\text{points}} + \lambda_{\text{normal}} L_{\text{normal}} + \lambda_{\text{conf}} L_{\text{conf}} + \lambda_{\text{cam}} L_{\text{cam}} $$

其中 $L\_{\text{normal}}$ 最小化预测与 GT 表面法向的夹角，论文取 $\lambda\_{\text{normal}}=1.0$ 、 $\lambda\_{\text{conf}}=0.05$ 、 $\lambda\_{\text{cam}}=0.1$ 。
...
| Confidence 头（独立） | 冻结主干，仅训 confidence 头 | — | — | — |

- **confidence 头单独训练**：这是一个容易漏掉但很关键的细节。confidence 头**不与其余模块联合训练**——两个主阶段完成后，冻结网络其余部分，单独训练 confidence 头。这解释了为什么 GitHub `training` 分支把流程写成「low-res / high-res / confidence branch」三阶段：前两阶段学几何，第三阶段在几何固定后学「哪里可信」。分离训练避免 confidence 目标（依赖当前几何误差）在几何还在剧烈变化时给出摇摆的监督信号。
...
- **全局点云如何拼**（我的解读，原文未逐步描述）：每张图的 $\hat{X}\_i$ 定义在自己的相机坐标系里，要拼成一朵统一点云，就需要把各图的局部点图用预测的相对位姿 $\hat{T}\_i$ 变换到同一参考系。由于 Pi3 输出的是 affine/scale-invariant 几何，这一步得到的是「相对正确、全局尺度待定」的点云；要 metric 尺度需外部约束或 Pi3X。原文重点在 formulation 与各任务指标，没有把这一后处理写成显式算法，因此归为推断而非事实。
