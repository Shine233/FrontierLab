...
- 符号： $\mathbf{r}(t)=\mathbf{o}+t\mathbf{d}$ 是从相机原点 $\mathbf{o}$ 沿方向 $\mathbf{d}$ 的光线； $[t_n,t_f]$ 是近/远采样边界； $\sigma$ 是体密度（单位长度上光线被吸收/散射的微分概率）； $\mathbf{c}$ 是该点视角相关颜色； $T(t)$ 是**累积透射率**——光线从 $t_n$ 走到 $t$ 未被遮挡的概率。
...
- 符号： $N$ 是光线上采样点数； $\delta_i=t_{i+1}-t_i$ 是相邻采样点间距； $\alpha_i$ 是第 $i$ 段的不透明度（alpha）； $T_i$ 是离散累积透射率。
...
- **采样**：coarse 网络均匀分层采样 $N_c=64$ 个点；fine 网络在 coarse 密度导出的分布上再采 $N_f=128$ 个点，与前 64 个合并（共 192 个点）参与 fine 渲染。
