...
整体结构（模块分解）：
- **图像编码器 $E\_I$**：ViT-Large（用 DUSt3R 权重初始化），patch 16×16，把每帧 $\mathbf{I}\_t$ 编成 token 特征 $\mathbf{F}\_t$ 。
...
数据流（每帧）： $\mathbf{I}\_t \to E\_I \to \mathbf{F}\_t \to$ 与 $\mathbf{s}\_{t-1}$ 在双解码器交互 $\to (\mathbf{F}'\_t, \mathbf{z}'\_t, \mathbf{s}\_t) \to$ 三个读出头 $\to$ 点图/位姿；同时 $\mathbf{s}\_t$ 写回作为下一帧输入。
