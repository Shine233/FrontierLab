...
- **输入 / 输出**：输入 query image $I\_q$ 、K 张检索到的 source/database images $\{I\_{d\_k}\}$ 以及这些 source images 的已知 6DoF poses $\{R,T\}\_{d\_k}$ ；输出 query 在 world coordinate system 下的绝对 pose $\{R,T\}\_q$ 。
...
- 符号： $I\_q$ 是 query image； $I\_{d\_k}$ 是第 $k$ 张 source image； $\{R,T\}\_{d\_k}$ 是该 source 的已知 6DoF pose； $f\_\theta$ 是以 VGGT 为骨干的网络； $\{R,T\}\_q$ 是待估的 query 绝对位姿。
